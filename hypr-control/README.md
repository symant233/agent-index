...
1. 在手机/其他设备浏览器打开 **`http://<主机IP>:8080`**（控制端口，默认 8080；本机可用 `http://localhost:8080`）。
   > 明文 HTTP 会自动 301 重定向到 `https://`（同一端口双协议），首次会提示自签名证书不受信任，选择“高级 → 继续访问”即可（证书 10 年有效，只需一次）。
...
> **设备授权持久化**：设备表保存在主机 `%LOCALAPPDATA%\hypr-control\devices.json`（白名单含 token）。
> 浏览器清缓存或 http/https 切换导致设备 ID 丢失时，服务端会按来源 IP+User-Agent 自动找回原授权，无需重新 allow。
